...
bool loadConfigurationFromFile(const std::string& configFilePath, ToolConfiguration& config) {
    ...
    while (std::getline(configFile, line)) {
        ...
        if (parseKeyValuePair(line, key, value)) {
            if (key == "input_directory") {
            ...
            } else if (key == "material_filter_mode") {
                ...
                if (modeLower == "none" || modeLower == "hash" || modeLower == "index") {
                    config.materialFilterMode = modeLower;
                } else {
                    GltfInstancing::logWarning("Invalid material_filter_mode '" + value + "'. Using 'none'. Valid: none, hash, index.");
                }
    ...
    while (argIndex < argc) {
        ...
        if (arg == "--input_directory") {
            ...
            if (argIndex + 1 < argc) {
                ...
                if (modeLower == "none" || modeLower == "hash" || modeLower == "index") {
                    config.materialFilterMode = modeLower;
                    GltfInstancing::logDebug("Command-line override: material_filter_mode = " + modeLower);
                } else {
                    GltfInstancing::logWarning("Invalid --material-filter-mode: " + mode + ". Valid: none, hash, index.");
                }
            } else {
                GltfInstancing::logError("--material-filter-mode requires a value.");
            }
        }
