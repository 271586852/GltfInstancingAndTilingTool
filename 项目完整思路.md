...
1. **融合哈希算法的多维度信息的相似性判别模型**：将**语义哈希**（`buildSemanticHashKey`）与**材质哈希**（`getMeshMaterialHash`）融入多维度判定流程，从几何、语义、材质、变换等维度建立构件等价判定体系。通过哈希驱动的语义分组降规模、材质哈希过滤避免误判、Hausdorff 几何相似度精化，实现高效且可配置的实例化检测。
...
**核心一**：建立**融合哈希算法的多维度信息的相似性判别模型**，以语义哈希、材质哈希与 Hausdorff 几何相似度协同，支撑高效实例化检测  
...
将 Hausdorff 距离等几何相似度度量引入 BIM 实例化检测，提出**融合哈希算法的多维度信息的相似性判别模型**。该模型将**语义哈希**（`buildSemanticHashKey`）与**材质哈希**（`getMeshMaterialHash`）融入判定流程：先通过语义哈希键对 category、family、type 进行可配置分组降规模，再通过材质哈希（PBR 因子、emissiveFactor、alphaMode、doubleSided）过滤材质差异大的构件，最后结合 Hausdorff 相似度判定几何等价。通过 `SemanticMaterialGeometricDetector` 实现多维度协同，解决了旋转、平移、均匀/非均匀缩放下的构件实例识别问题，突破了传统单一依赖语义 ID 或严格几何匹配的局限性。
...
### 核心一：融合哈希算法的多维度信息的相似性判别模型

BIM 构件的实例化等价判定需综合考虑**几何、语义、材质、变换**等多维度信息。本项目建立了一套**融合哈希算法的多维度信息的相似性判别模型**，将**哈希算法**作为多维度信息融合的桥梁：语义哈希实现粗粒度分组、材质哈希实现细粒度过滤、Hausdorff 实现几何精化。通过 `SemanticMaterialGeometricDetector` 实现高效、可配置的实例化检测。

#### 3.1.0 哈希驱动的多维度信息融合框架

**哈希算法在模型中的双重作用**：

| 哈希类型 | 作用 | 输入 | 输出 | 代码对应 |
|----------|------|------|------|----------|
| **语义哈希** | 将 category、family、type 等拼接为分组键，实现粗粒度预筛选 | RISCRVT 语义属性 | 分组键（字符串） | `buildSemanticHashKey` |
| **材质哈希** | 将 PBR 因子、emissiveFactor 等编码为哈希值，实现材质一致性过滤 | glTF Material | `size_t` 哈希值 | `getMeshMaterialHash` |

**维度划分与哈希融合**：

| 维度 | 信息内容 | 哈希融合方式 | 代码对应 |
|------|----------|--------------|----------|
| **语义维度** | Category、Family、Type | 语义哈希键分组，$O(n \log n)$ 降规模 | `buildSemanticHashKey`、`semantic_hash_fields` |
| **材质维度** | PBR、emissiveFactor、alphaMode | 材质哈希相等才进入几何比较 | `getMeshMaterialHash`、`material_filter_mode=hash` |
| **几何维度** | 顶点、点云、Hausdorff 距离 | 哈希过滤后的候选对进行 Hausdorff 精化 | `computeMeshSimilarity`、`hausdorff_similarity.cpp` |
| **变换维度** | 平移、旋转、缩放 | 通过 `MeshInstanceInfo` 记录 | `TransformComponents` |

**融合策略**：先按语义哈希键（`semanticHashFields`）分组；组内按材质哈希（`material_filter_mode=hash` 时）过滤；最后用 Hausdorff 相似度（`similarity_threshold`）判定几何等价；变换通过 `MeshInstanceInfo` 记录实例位置。
...
| 层级 | 方法 | 哈希作用 | 涉及维度 | 复杂度 |
|------|------|----------|----------|--------|
| 语义预筛选 | 按 C/F/T 分组 | 语义哈希键 `buildSemanticHashKey` | 语义 | O(n log n) |
| 材质过滤 | material_filter_mode（可选） | 材质哈希 `getMeshMaterialHash` 相等才比较 | 材质 | O(n_i) |
| Hausdorff 精化 | 点云采样 + Hausdorff 距离 → 相似度 | 哈希过滤后的候选对 | 几何 | O(k·m²) |
...
- `similarity_thresholds`：逗号分隔，如 `0.95,0.90,0.85,0.80,0.75`，主流程实例化检测使用首值（如 0.95）
- HLOD 使用 `hlod_similarity_thresholds`（如 `0.70,0.65,0.60,0.55,0.50`），每层级可配置不同阈值

**3. 材质过滤（可选）**

- `material_filter_mode = "none"`：不按材质过滤，仅几何+语义
- `material_filter_mode = "hash"`：材质 hash 相同才比较（PBR 因子、emissiveFactor、alphaMode、doubleSided，不包含纹理引用）
...
- **HLOD**：`hlod_similarity_thresholds`、`hlod_instance_limit`（可独立于 Stage1）
...
第三章 融合哈希算法的多维度信息的相似性判别模型（核心一）
  3.1 多维度信息定义与哈希融合框架
  3.2 语义哈希分组与 Hausdorff 几何相似度判定
  3.3 材质哈希过滤与变换等价
...
**核心贡献一：融合哈希算法的多维度信息的相似性判别模型**

1. **哈希驱动的多维度融合**：将语义哈希与材质哈希融入判定流程，建立几何、材质、语义、变换四维度的等价判定体系。语义哈希（`buildSemanticHashKey`）实现可配置分组降规模，材质哈希（`getMeshMaterialHash`）实现材质一致性过滤，Hausdorff 实现几何精化
2. **哈希-几何协同筛选**：先语义哈希分组降规模，再材质哈希过滤避免误判，最后 Hausdorff 相似度精化，兼顾效率与召回率
