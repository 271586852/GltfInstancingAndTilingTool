...
*   **步骤 2：命令行封装**
    *   修改 C++ 的 `main` 函数，使其能够解析命令行参数。例如，可以这样调用它：
      `./tool.exe --input_directory "path/to/model.glb" --output_directory "path/to/output_dir/" --similarity-thresholds 0.95,0.90,0.85`
    *   当前 C++ 工具已支持 `--input_directory`、`--output_directory`、`--similarity-thresholds`、`--semantic-hash-fields`、`--hausdorff-max-sample-points`、`--instance-limit` 等参数。
