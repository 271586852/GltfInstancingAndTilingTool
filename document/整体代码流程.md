...
3.  **检测实例化机会 (`SemanticMaterialGeometricDetector` 模块 - `semantic_material_geometric_detector.h/cpp`)**:
    *   `SemanticMaterialGeometricDetector::detect()`: 接收加载的 `LoadedGltfModel` 列表。
        *   **语义分组**: 按 `semanticHashFields`（category、family、type）生成语义键，相同键的 Mesh 进入同一候选组。数据来源：`SemanticParser` 解析 RISCRVT XML。
        *   **材质过滤**（可选）: `material_filter_mode` 为 hash 或 index 时，仅材质匹配的 Mesh 才比较。
        *   **Hausdorff 几何相似度判定**:
            *   对 Mesh 顶点进行点云采样（`hausdorff_max_sample_points` 限制规模）。
            *   调用 `computeMeshSimilarity` 计算双向 Hausdorff 距离，转换为相似度 `1/(1+distance)`。
            *   相似度 ≥ `similarity_threshold` 的 Mesh 归入同一实例组。
        *   **场景图遍历与实例收集**:
            *   `traverseNode()`: 递归遍历每个 `LoadedGltfModel` 中的场景图，按语义键分组。
            *   组内通过 Hausdorff 相似度判定，将几何等价的实例收集到 `InstancedMeshGroup`。
        *   **结果分类**: 实例数 ≥ `instanceLimit` 的组归类为 `InstancedMeshGroup`，否则归类为 `NonInstancedMeshInfo`。
...
4.  **生成包含实例化的新 GLB 文件 (`GlbWriter` 模块 - `glb_writer.h/cpp`)**:
    *   `GlbWriter::writeInstancedGlb()`: 接收原始的 `LoadedGltfModel` 列表（用于访问原始数据）和 `InstancingDetectionResult`（由 `SemanticMaterialGeometricDetector::detect()` 生成）。输出路径为 `01_instancing/instanced.glb`、`non_instanced.glb` 等。
...
**3. `semantic_material_geometric_detector.h` / `semantic_material_geometric_detector.cpp` (`GltfInstancing` 命名空间)**

*   **功能**: 分析已加载的 GLTF 模型，按语义分组 + Hausdorff 几何相似度识别可实例化的 Mesh，并收集实例信息。
*   **主要内容**:
    ...
    *   `class SemanticMaterialGeometricDetector`:
        *   `SemanticMaterialGeometricDetector(SemanticParser*, semanticHashFields, similarityThreshold, ...)`: 构造函数。需传入 `SemanticParser`、`similarityThresholds`、`hausdorffMaxSamplePoints` 等。
        *   `detect(const std::vector<LoadedGltfModel>&)`: 主要的检测逻辑入口。
            *   `traverseNode(...)` (private): 递归遍历场景节点，按语义键（`buildSemanticHashKey`）分组。
            *   组内调用 `computeMeshSimilarity`（点云采样 + Hausdorff 距离 → 相似度）。
            *   相似度 ≥ `similarity_threshold` 的 Mesh 归入同一 `InstancedMeshGroup`。
            *   实例数 < `instanceLimit` 的组解散为 `NonInstancedMeshInfo`。
        *   `buildSemanticHashKey(const std::optional<SemanticInfo>&)` (private): 按 category、family、type 生成语义键。
        *   依赖 `hausdorff_similarity.cpp` 中的 `computeHausdorffDistance`、`hausdorffDistanceToSimilarity`。
...
*   **主要内容**:
    ...
    *   `main(int argc, char* argv[])`:
        ...
        *   创建 `SemanticMaterialGeometricDetector` 实例，调用其方法进行实例化检测。
